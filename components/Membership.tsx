...
const Membership = ({ harga, jangka, bayaran, onPress, selected }: any) => {
  const getHematText = (jangka: string) => {
    if (jangka === '12 Bulan') return 'Hemat 42%';
    if (jangka === '4 Bulan') return 'Hemat 53%';
    if (jangka === '1 Bulan') return 'Hemat 100%';
    return '';
  };
  ...
  return (
    <TouchableOpacity onPress={onPress}>
      <View
        style={tw`mt-5 bg-white shadow-lg rounded-lg p-5 flex-row justify-between items-center ${
      ...
      >
        ...
        <View style={tw`flex-col justify-between items-end`}>
          ...
          <Text style={tw`font-semibold`}>{bayaran}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
